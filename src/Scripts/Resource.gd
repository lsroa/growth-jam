...
func _ready():
	...
	for adjancent_building_position in random_adjancent_building_positions:
		var adjacent_coordinates = adjacent_coordinates(_initial_position, adjancent_building_position)
		var adjacent_building = preload("res://Scenes/AdjacentBuilding.tscn").instance()
		adjacent_building.init("(%s,%s): %s" % [_column, _row, adjancent_building_position], adjacent_coordinates)
